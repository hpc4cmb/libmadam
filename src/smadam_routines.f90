...
CONTAINS
  ...
  SUBROUTINE pixel_matrix(cca, cc)
    ...
    integer :: i, ip, n, ierr, idet, num_threads, ichunk, noba, kstart, ipsd, k, firstpix, lastpix
    ...
    !$OMP          dummy_pixel,loccc,weights,nthreads,nsize_locmap,detflags,surveyflags) &
    ...
    if ( id == 0 .and. nthreads /= omp_get_num_threads() ) &
         print *,'WARNING : building pixel matrices with only ',omp_get_num_threads(),' / ',nthreads,' threads'
          ...
          do ichunk = first_chunk, last_chunk
             ...
             do k = kstart+1,kstart+noba
                do i = baselines_short_start(k), baselines_short_stop(k)
                   ...
                   !if (ip == dummy_pixel) cycle
                   if ( ip < firstpix .or. ip > lastpix ) cycle
                   call dsyr( 'U', nmap, detweight, weights(:,i,idet), 1, loccc(:,:,ip), nmap )
                end do
             end do
          end do
       end if
  ...
  SUBROUTINE bin_tod(map, binmap, wamap, tod_stored)
    ...
    integer :: i, n, ip, m, ierr, firstpix, lastpix, idet, ichunk, noba, kstart, ipsd, k
    ...
    !$OMP     SHARED(nsize_locmap,nodetectors,detflags,nmap,first_chunk,last_chunk,&
    !$OMP            noba_short_pp,detectors,baselines_short_start,baselines_short_stop,&
    !$OMP            isubchunk,subchunkpp,pixels,dummy_pixel,baselines_short_time,surveyflags,&
    !$OMP            tod_stored,weights,nthreads,locmap,id,nosamples_proc)

    if ( id == 0 .and. nthreads /= omp_get_num_threads() ) &
         print *,'WARNING : binning TOD with only ',omp_get_num_threads(),' / ',nthreads,' threads'
          ...
          do ichunk = first_chunk, last_chunk
             ...
             do k = kstart+1,kstart+noba
                do i = baselines_short_start(k), baselines_short_stop(k)
                   ...
                   !if (ip == dummy_pixel) cycle
                   if ( ip < firstpix .or. ip > lastpix ) cycle
                   locmap(:,ip) = locmap(:,ip) + weights(:,i,idet) * tod_stored(i,idet) * detweight
                end do
             end do
          end do
       end if
  ...
  SUBROUTINE count_hits(nohits)
    ...
    do idet = 1,nodetectors
       ...
       !$OMP PARALLEL DEFAULT(NONE) NUM_THREADS(nthreads) &
       !$OMP   SHARED(nodetectors,first_chunk,last_chunk,noba_short_pp,baselines_short_time,&
       !$OMP          baselines_short_start,baselines_short_stop,isubchunk,subchunkpp,surveyflags,&
       !$OMP          lochits,dummy_pixel,do_dethits,nohits,nosubpix_map,idet,nsize_locmap,nthreads,&
       !$OMP          detectors,pixels,id,nopix_map) &
       !$OMP   PRIVATE(ichunk,noba,kstart,ipsd,detweight,k,i,ip,firstpix,lastpix,id_thread)
       
       if ( id == 0 .and. nthreads /= omp_get_num_threads() ) &
            print *,'WARNING : counting hits with only ',omp_get_num_threads(),' / ',nthreads,' threads'
  ...
  SUBROUTINE initialize_a(yba, nna, wamap, cca, tod_stored)
    ...
    integer(i8b) :: i, k, ip, j, order, order2, i0, imap, n, workspace_length, ntot, nbad, ngood
    integer(i4b) :: ichunk, noba, kstart, ipsd, idet
    real(dp), pointer :: basis_function(:,:)
    REAL(dp), allocatable :: workspace(:), eigenvectors(:,:), eigenvectorsT(:,:), eigenvalues(:)
    integer :: ierr
       ...
       do ichunk = first_chunk, last_chunk
          ...
          do k = kstart+1,kstart+noba
             ...
             do i = baselines_short_start(k), baselines_short_stop(k)
                ...
                ! Use of locmask rather than locmap==0 agrees with the old versions
                ! of Madam and does not exclude poorly conditioned pixels.
                if ( all(locmap(:,ip) == 0) ) cycle ! masked by inmask or pixel rejection
                !if ( use_inmask .and. locmask(ip) == 0 ) cycle
                
                do order = 0,basis_order
                   bf = basis_function( order, i-i0 )
                   yba(order, k, idet) = yba(order, k, idet) + bf * tod_stored(i,idet)
                   nna(:, order, k, idet) = nna(:, order, k, idet) + bf * basis_function(:,i-i0)
                end do
                ...
                do order = 0,basis_order
                   ...
                   if (nmap == 1) then
                      ...
                      yba(order,k,idet) = yba(order,k,idet) - bf * dot_product(locmap(:,ip), weights(:,i,idet))
                   end if
                end do
             end do

             yba(:,k,idet) = yba(:,k,idet) * detweight
             nna(:,:,k,idet) = nna(:,:,k,idet) * detweight ! Stock version applied detweight later
       ...
       do ichunk = first_chunk, last_chunk
          ...
          do k = kstart+1,kstart+noba
             ...
             if ( abs(nna(0,0,k,idet)) < 1e-30 ) then
                ...
                call dsyev( 'V', 'U', n, eigenvectors, n, eigenvalues, workspace, workspace_length, ierr)
             end if
             bad_baseline = .true.
             if (ierr /= 0) then
                ...
                yba(:,k,idet) = 0
                nbad = nbad + 1
             else if (minval(eigenvalues) == 0 .and. maxval(eigenvalues) == 0) then
                nna(:,:,k,idet) = 0
                yba(:,k,idet) = 0
                cycle
  ...
  SUBROUTINE iterate_a(aa, yba, nna, wamap, cca, tod_stored)
    ...
    integer :: i, j, k, n, m, ip, istep, idet, first, last, order, i0, m0, order2, ichunk, noba, kstart, ipsd, itask
    real(dp), pointer :: basis_function(:,:)

    ! for openmp -RK
    integer :: ierr, imap
    real(dp), allocatable, target :: locmap_all_threads(:,:,:), ap_all_threads(:,:,:,:)
          ...
          do ichunk = first_chunk, last_chunk
             ...
             do k = kstart+1,kstart+noba
                ...
                do m = baselines_short_start(k), baselines_short_stop(k)
                   ...
                   pw = dot_product(basis_function(:,m-m0),aa(:,k,idet)) * detweight
                   if (nmap == 1) then
             ...
             do k = kstart+1,kstart+noba
                do order = 0,basis_order
                   ...
                   do i = baselines_short_start(k), baselines_short_stop(k)
                      ...
                      if (nmap == 1) then
                         apn = apn - locmap(1,ip) * basis_function(order,i-i0) * detweight
                      else
                         apn = apn - dot_product(weights(:,i,idet), locmap(:,ip)) * basis_function(order,i-i0) * detweight
                      end if
                   end do
       ...
       do idet = 1,nodetectors
          do i = 1,noba_short
             z(0:basis_order,i,idet) = matmul( &
                  nna_inv(0:basis_order,0:basis_order,i,idet), r(0:basis_order,i,idet))
          end do
       end do
    end if
          ...
          do i = 1,noba_short
             do order = 0, basis_order
                if (isnan(yba(order, i, idet))) then
                   print *,id,' : yba has NaN(s) : idet, base, order : ', idet, i, order
                   exit ybaloop
                end if
             end do
          end do
          ...
          do i = 1,noba_short
             do order = 0, basis_order
                if (isnan(r(order,i,idet))) then
                   print *,id,' : r has NaN(s) : idet, base, order : ', idet, i, order
                   exit rloop
                end if
             end do
          end do
          ...
          do i = 1,noba_short
             do order = 0, basis_order
                if (isnan(z(order,i,idet))) then
                   print *,id,' : z has NaN(s) : idet, base, order : ', idet, i, order
                   print *,id,' : nna_inv = ',nna_inv(0:basis_order,0:basis_order,i,idet)
          ...
          do i = 1,noba_short
             do order = 0, basis_order
                if (isnan(p(order,i,idet))) then
                   print *,id,' : p has NaN(s) : idet, base, order : ', idet, i, order
                   exit ploop
                end if
             end do
          end do
    ...
    if (rzinit.lt.1.e-30) then
       if (id == 0) then
          write(*,*)
          write(*,'(x,a,es25.15,a)') 'Anomalously low rzinit =',rzinit,' NO CG ITERATION'
       end if
       return
    end if
    ...
    allocate(ap_all_threads(0:basis_order,noba_short,nodetectors,0:nthreads-1), stat=ierr)
    ...
    do
       ...
       aa(:, 1:noba_short, 1:nodetectors) = aa(:, 1:noba_short, 1:nodetectors) + alpha*p
  ...
  contains
    ...
    subroutine baseline_to_map_order0_nopol()
      ...
      !$OMP   PRIVATE(itask,id_thread,locmap_thread,idet,ichunk,noba,kstart,ipsd,detweight,k,pw,m,ip)
      itask = -1
      id_thread = omp_get_thread_num()
      ...
      loop_detector : do idet = 1, nodetectors
         loop_chunk : do ichunk = first_chunk, last_chunk
            ...
            itask = itask + 1
            if ( modulo( itask, omp_get_num_threads() ) /= id_thread ) cycle loop_chunk
         ...
         loop_chunk : do ichunk = first_chunk, last_chunk
            ...
            loop_baseline : do k = kstart+1,kstart+noba
               ...
               do m = baselines_short_start(k), baselines_short_stop(k)
                  ...
                  locmap_thread(1,ip) = locmap_thread(1,ip) + weights(1,m,idet) * pw
                  locmap_thread(2,ip) = locmap_thread(2,ip) + weights(2,m,idet) * pw
                  locmap_thread(3,ip) = locmap_thread(3,ip) + weights(3,m,idet) * pw                         
               end do
            end do loop_baseline
         end do loop_chunk
      end do loop_detector
         ...
         loop_chunk : do ichunk = first_chunk, last_chunk
            ...
            loop_baseline : do k = kstart+1,kstart+noba
               ...
               do m = baselines_short_start(k), baselines_short_stop(k)
                  ...
                  locmap_thread(:,ip) = locmap_thread(:,ip) + weights(:,m,idet) * pw
               end do
            end do loop_baseline
         end do loop_chunk
      end do loop_detector
    ...
    subroutine baseline_to_map_nopol()
      ...
      !$OMP          noba_short_pp,baselines_short_time,detectors,nthreads,p,&
      !$OMP          baselines_short_start,baselines_short_stop,&
      !$OMP          basis_functions,basis_order,isubchunk,subchunk,pixels,dummy_pixel) &
      !$OMP   PRIVATE(itask,id_thread,locmap_thread,idet,ichunk,noba,kstart,ipsd,detweight,k,m0,basis_function,m,ip)
      itask = -1
      id_thread = omp_get_thread_num()
      locmap_thread => locmap_all_threads(:,:,id_thread)
      loop_detector : do idet = 1, nodetectors
            ...
            loop_baseline : do k = kstart+1,kstart+noba
               ...
               do m = baselines_short_start(k), baselines_short_stop(k)
                  ...
                  locmap_thread(1,ip) = locmap_thread(1,ip) + detweight &
                       * dot_product(basis_function(0:basis_order,m-m0), p(0:basis_order,k,idet))
               end do
            end do loop_baseline
         end do loop_chunk
      end do loop_detector
    ...
    subroutine baseline_to_map_general()
      ...
      !$OMP          noba_short_pp,baselines_short_time,detectors,nthreads,p,&
      !$OMP          baselines_short_start,baselines_short_stop,&
      !$OMP          basis_functions,basis_order,isubchunk,subchunk,pixels,dummy_pixel,nmap,weights) &
      !$OMP   PRIVATE(itask,id_thread,locmap_thread,idet,ichunk,noba,kstart,ipsd,detweight,k,m0,basis_function,m,ip,pw)
      itask = -1
      id_thread = omp_get_thread_num()
      locmap_thread => locmap_all_threads(:,:,id_thread)
      loop_detector : do idet = 1, nodetectors
         loop_chunk : do ichunk = first_chunk, last_chunk
            ...
            loop_baseline : do k = kstart+1,kstart+noba
               ...
               do m = baselines_short_start(k), baselines_short_stop(k)
                  ...
                  pw = dot_product(basis_function(0:basis_order,m-m0),p(0:basis_order,k,idet)) * detweight
                  locmap_thread(1:nmap,ip) = locmap_thread(1:nmap,ip) + weights(1:nmap,m,idet)*pw
               end do
            end do loop_baseline
         end do loop_chunk
      end do loop_detector
    ...
    subroutine map_to_baseline_order0_nopol()
      ...
      !$OMP   PRIVATE(itask,id_thread,ap_thread,idet,ichunk,noba,kstart,ipsd,detweight,k,apn,i,ip)
      itask = -1
      id_thread = omp_get_thread_num()
      ...
      loop_detector_ap : do idet = 1, nodetectors
         loop_chunk_ap : do ichunk = first_chunk, last_chunk
            ...
            itask = itask + 1
            if ( modulo( itask, omp_get_num_threads() ) /= id_thread ) cycle loop_chunk_ap
            
            loop_baseline_ap : do k = kstart+1,kstart+noba
               apn = nna(0,0,k,idet) * p(0,k,idet) / detweight ! / detweight is canceled later
         ...
         loop_chunk_ap : do ichunk = first_chunk, last_chunk
            ...
            loop_baseline_ap : do k = kstart+1,kstart+noba
               ...
               do i = baselines_short_start(k), baselines_short_stop(k)
                  ...
                  apn = apn - dot_product(weights(1:nmap,i,idet), locmap(1:nmap,ip))
               end do
    ...
    subroutine map_to_baseline_nopol()
      ...
      !$OMP          nna,p,baselines_short_start,baselines_short_stop,&
      !$OMP          isubchunk,subchunk,pixels,dummy_pixel,weights,locmap) &
      !$OMP   PRIVATE(itask,id_thread,ap_thread,idet,ichunk,noba,kstart,ipsd,detweight,k,apnv,i0,basis_function,i,ip)
      itask = -1
      id_thread = omp_get_thread_num()
      ap_thread => ap_all_threads(:,:,:,id_thread)
      ...
      loop_detector_ap : do idet = 1, nodetectors
         loop_chunk_ap : do ichunk = first_chunk, last_chunk
            ...
            loop_baseline_ap : do k = kstart+1,kstart+noba
               apnv = matmul( nna(0:basis_order,0:basis_order,k,idet), p(0:basis_order,k,idet) ) / detweight ! / detweight is canceled later
         ...
         loop_chunk_ap : do ichunk = first_chunk, last_chunk
            ...
            loop_baseline_ap : do k = kstart+1,kstart+noba
               ...
               do i = baselines_short_start(k), baselines_short_stop(k)
                  ...
                  apnv = apnv - dot_product(weights(1:nmap,i,idet), locmap(1:nmap,ip)) * basis_function(0:basis_order,i-i0)
               end do
    ...
    do idet = 1,nodetectors
       do k = 1,noba_short
          ...
          do i = baselines_short_start(k), baselines_short_stop(k)
             if ( isubchunk /= 0 .and. subchunkpp(i) /= isubchunk ) cycle
             do order = 0, basis_order
                tod(i,idet) = tod(i,idet) - basis_function(order,i-i0) * aa(order,k,idet)
             end do
          end do
       end do
    end do
