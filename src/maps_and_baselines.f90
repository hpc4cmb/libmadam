MODULE maps_and_baselines
  ...
  integer, allocatable, public :: nnb(:,:)
  real(dp), allocatable, public :: nna(:,:,:,:), nna_inv(:,:,:,:)
  real(dp), allocatable, public :: aa(:,:,:), ab(:,:)
  real(dp), allocatable, public :: yba(:,:,:)

  integer, allocatable, public :: inmask(:), outmask(:)
  integer, allocatable, public :: nohits(:,:)
  real(dp),allocatable, public :: cca(:,:,:), cc(:,:,:)
  real(dp),allocatable, public :: wamap(:,:), wbmap(:,:)
  real(dp),allocatable, public :: map(:,:), binmap(:,:), wnmap(:,:)
...
CONTAINS
  ...
  SUBROUTINE allocate_maps
    ...
    if (use_inmask) then
       ...
       if (.not. allocated(inmask)) allocate(inmask(0:0)) ! -RK
    endif
    ...
    if (kfirst) then
       ...
       allocate(cca(nmap,nmap,0:0), wamap(nmap,0:0)) ! -RK
    endif

    memsum = memory_maps/1024./1024.
    ...
    if (ID==0.and.info.ge.1) write(*,mstr3) 'Allocated memory for maps:',memsum, mem_min, mem_max ! -RK
  ...
  SUBROUTINE free_maps
    ...
    if (allocated(wnmap))   deallocate(wnmap)
  ...
  SUBROUTINE allocate_baselines
    ...
    if (kfirst) then
       ...
       nna = 0
       nna_inv = 0
       !FIXME Check memory consumption
       memory_baselines = memory_baselines + nodetectors*noba_short_max*(basis_order+1)*16
       memory_baselines = memory_baselines + nodetectors*noba_short_max*(basis_order+1)**2*16

    else ! -RK
       allocate(yba(1,1,1), nna(1,1,1,1)) ! -RK
    endif

    memsum = memory_baselines/1024./1024.
    ...
    if (ID==0.and.info.ge.1) write(*,mstr3)  &
