...
MODULE covmat
  ...
  real(sp), save, public :: memory_ncm = 0
  real(sp), save, public :: cputime_ncm=0, cputime_hitmap=0
  real(sp), save, public :: cputime_ptf=0, cputime_middlematrix=0, cputime_accumulate=0
  real(sp), save, public :: cputime_white=0, cputime_save_matrix=0, cputime_symmetrize=0
  real(sp), save, public :: cputime_invert_middlematrix=0, cputime_symmetrize_middlematrix=0

  real(dp), allocatable :: local_covmat(:,:,:,:), local_ptf(:,:,:)
  real(dp), allocatable :: middlematrix(:,:)
...
contains
  ...
  subroutine write_covmat(outroot)
    ...
    allocate( local_covmat(nmap,nmap,0:nolocpix,0:nolocpix), local_hitmap(0:nolocpix), stat=ierr )
    if ( ierr /= 0 ) then
       print *,'Failed to allocate ',(nmap**2*(nolocpix+1)**2*8 + (nolocpix+1)*4)/2**20,' MB for local_covmat. nolocpix = ',nolocpix
    ...
    loop_detector : do idet = 1,nodetectors
       ...
       outfile = trim(outroot) // '_' // trim(detectors(idet)%name) // '_inv_bin.dat'
             ...
             loop_pix2 : do jpix = ipix,nolocpix-1
                ...
                loop_baseline : do i = 1, local_hitmap(ipix)
                   ...
                   loop_baseline2 : do j = 1, local_hitmap(jpix)
                      ...
                      if (nmap == 1) then
                         local_covmat(1,1,jpix,ipix) = local_covmat(1,1,jpix,ipix) &
                              + mm * local_ptf(1,ibase,ipix) * local_ptf(1,jbase,jpix)
                      else
                         ptf1 = mm * local_ptf(1,ibase,ipix)
                         local_covmat(1:3,1,jpix,ipix) = local_covmat(1:3,1,jpix,ipix) &
                              + local_ptf(1:3,jbase,jpix) * ptf1

                         ptf1 = mm * local_ptf(2,ibase,ipix)
                         local_covmat(1:3,2,jpix,ipix) = local_covmat(1:3,2,jpix,ipix) &
                              + local_ptf(1:3,jbase,jpix) * ptf1

                         ptf1 = mm * local_ptf(3,ibase,ipix)
                         local_covmat(1:3,3,jpix,ipix) = local_covmat(1:3,3,jpix,ipix) &
                              + local_ptf(1:3,jbase,jpix) * ptf1
                      end if
       ...
       if (id == 0) print *,'Saving the matrix to ', trim(outroot) // '_' // trim(detectors(idet)%name) // '_inv_bin.dat'
  ...
  subroutine add_white(idet, detweight)
    ...
    do k = 1,noba_short
       loop_sample : do i = baselines_short_start(k), baselines_short_stop(k)
          ...
          !call dsyr( 'U', nmap, detweight, weights(:,i,idet), 1, local_covmat(:,:,ip,ip), nmap )
          call dger( nmap, nmap, detweight, weights(:,i,idet), 1, &
               weights(:,i,idet), 1, local_covmat(:,:,ip,ip), nmap )
          !do imap = 1,nmap
          !   do jmap = 1,nmap
          !      local_covmat(jmap,imap,ip,ip) = local_covmat(jmap,imap,ip,ip) &
          !           + weights(imap,i,idet) * weights(jmap,i,idet) * detweight
          !   end do
          !end do
       end do loop_sample
    end do
    ...
    do ipix = 0,nolocpix-1
       do jpix = ipix+1,nolocpix-1
          if ( local_covmat(1,1,jpix,ipix) /= 0 ) then
             local_covmat(:,:,ipix,jpix) = transpose(local_covmat(:,:,jpix,ipix))
          end if
       end do
    end do
  ...
  subroutine save_matrix(covmatfile)
    ! create a global pixel-pixel matrix, accumulate the local contributions and write the matrix out
    character(len=*) :: covmatfile
    integer(i4b) :: ierr, mypix1, mypix2, row, col
    integer(i4b) :: isend, map1, map2, pix1, pix2, ip1, ip2, evenodd, itarget, isource, nelem
    ...
    INTEGER(i4b) :: filemode, fileinfo, outfilehandle, count, status(MPI_STATUS_SIZE)
    ...
    allocate( covmat1(npix*nmap, npix_proc*nmap), covmat2(npix*nmap, npix_proc*nmap), stat=ierr)
    if ( ierr /= 0 ) then
       print *,'Failed to allocate ',npix*nmap*npix_proc*nmap*2*8/2**20,' MB for collecting matrix'
    ...
    lastpix = firstpix + npix_proc - 1 ! It is ok to have lastpix > npix. No process will have entries to it
    ...
    cycle_matrices : DO isend = 0, ntasks - 1
       ...
       loop_col : do mypix1 = 0, nolocpix-1
          ...
          loop_row : do mypix2 = 0, nolocpix-1
             ...
             covmat_send(row:row+2,col:col+2) = covmat_send(row:row+2,col:col+2) &
       ...
       ELSE
          DO evenodd = 0, 1
             IF (MODULO(ID, 2) == evenodd) THEN
                ...
                CALL MPI_send(covmat_send, nelem, MPI_DOUBLE_PRECISION, itarget, ID, comm, ierr)
             ELSE
                !receive
                isource = MODULO(ID-1, ntasks)
                CALL MPI_recv(covmat_recv, nelem, MPI_DOUBLE_PRECISION, isource, isource, comm, status, ierr)
             END IF
          END DO
    ...
    CALL mpi_file_open(comm, TRIM(covmatfile), filemode, fileinfo, outfilehandle, ierr)
    ...
    CALL mpi_file_set_view(outfilehandle, fileoffset, MPI_REAL8, MPI_REAL8, 'native', fileinfo, ierr)
    IF (ierr /= 0) CALL abort_mpi('Unable to establish view to covmat file')

    count = npix * nmap * (lastpix - firstpix + 1) * nmap
    if ( count > 0 ) then
       CALL mpi_file_write(outfilehandle, covmat_recv, count, MPI_REAL8, status, ierr)
  ...
  subroutine get_hitmap(idet, kstart, noba)
    ...
    if ( ierr /= 0 ) then
       print *,'Failed to allocate ',hitmax*(nolocpix+1)*4/2**20,' MB for hitmap'
    ...
    ! local_basehitmap will have a list of all baselines that hit a particular pixel
       ...
       do i = baselines_short_start(k+kstart), baselines_short_stop(k+kstart)
          ...
          if (nhit == 0) then
             ...
             if (local_basehitmap(nhit,ip) /= k) then
                nhit = nhit + 1
                if ( nhit > hitmax ) then
                   print *,id,' : extra hits! ',k,i,nhit,hitmax,ip,nolocpix
                end if
  ...
  subroutine get_ptf(idet, kstart, noba, detweight)
    ...
    if ( ierr /= 0 ) then
       print *,'Failed to allocate ',nmap*noba*(nolocpix+1)*8/2**20,' MB for PTF. noba = ',noba,', nolocpix = ',nolocpix
       call abort_mpi( 'No room for local P^T F' )
    end if
  ...
  subroutine get_middlematrix(fcov, ipsd, idet, detweight, kstart, noba)
    ...
    complex(dp) :: fcov(nof/2+1,*)
    integer(i4b) :: ipsd, idet
    real(dp) :: detweight
    integer(i4b) :: kstart, noba
    ...
    if ( nof < 2 * noba ) call abort_mpi( 'ERROR: filter is too short to reliably estimate the middle matrix' )
    ...
    if ( bfinvert ) then
       ...
       if ( ierr /= 0 ) then
          print *,'Failed to allocate ',noba*noba*8/2**20,' MB for middlematrix. noba = ',noba
       ...
       if ( ierr /= 0 ) then
          print *,'Failed to allocate ',noba*8/2**20,' MB for middlematrix. noba = ',noba
          call abort_mpi( 'No room for local middlematrix' )
       end if
       if (noba > noba_max) then
    ...
    if (.not. bfinvert) then
       ...
       ! Adding the white noise term in Fourier domain means adding a constant offset
       ...
       if ( any( fcov(:,ipsd) == 0 ) ) then
          print *,id,' : WARNING : fcov had zero modes, regularizing for brute force inverse'
       ...
       cputime_invert_middlematrix = cputime_invert_middlematrix + get_time_and_reset(10)
       
       ! add the white noise diagonal

       do ibase = 1, noba
          middlematrix(ibase,ibase) = middlematrix(ibase,ibase) + nna(0,0,kstart+ibase,idet)
       end do
       cputime_middlematrix = cputime_middlematrix + get_time_and_reset(10)
       ...
       cputime_symmetrize_middlematrix = cputime_symmetrize_middlematrix + get_time_and_reset(10)
